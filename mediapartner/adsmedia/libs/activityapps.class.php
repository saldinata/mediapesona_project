<?php
	...
	class ActivityApps
	{
		...
		public function reqstacomment ( $id_berita )
		{
			...
			$query
				             = "SELECT * FROM tbl_statistic_comment_news WHERE id_berita=?";
			...
			if ( empty( $get_last_number ) )
			{
				$nominal = 0;
				$query
				         = "INSERT INTO tbl_statistic_comment_news(nominal,id_berita) VALUES(?,?)";
				$result  = $this -> db -> insertValue (
					$query ,
					[
						$nominal ,
			...
			else
			{
				...
				$query
					    = "UPDATE tbl_statistic_comment_news SET nominal=? WHERE id_statistic_comment_news=?";
		...
		{
			$query
				         = "SELECT * FROM tbl_comment WHERE id_berita=? ORDER BY id_comment DESC";
		...
		public function getCommentContent ( $id_comment )
		{
			...
			$query
				                    = "SELECT * FROM tbl_comment WHERE id_comment=? ORDER BY id_comment DESC";
		...
		public function reqstareadnews ( $id_berita )
		{
			...
			if ( empty( $get_last_number ) )
			{
				$nominal = 0;
				$query
				         = "INSERT INTO tbl_statistic_read_news(nominal,id_berita) VALUES(?,?)";
				$result  = $this -> db -> insertValue (
					$query ,
					[
						$nominal ,
			...
			else
			{
				...
				$query
					    = "UPDATE tbl_statistic_read_news SET nominal=? WHERE id_statistic_read_news=?";
				$result = $this -> db -> updateValue (
					$query ,
					[
						$nominal ,
		...
		public function getStaReadNews ( $id_berita )
		{
			$id_berita      = $this -> util -> sanitation ( $id_berita );
			$query
			                = "SELECT * FROM tbl_statistic_read_news WHERE id_berita=?";
